...
def init(dir_name: Path = typer.Argument()):
  ...
  shutil.copytree(f"{Path(__file__).parent}/samples", f"{dest_dir}/samples", dirs_exist_ok = True)
  shutil.copytree(f"{Path(__file__).parent}/templates", f"{dest_dir}/templates", dirs_exist_ok = True)
  shutil.copytree(f"{Path(__file__).parent}/.vscode", f"{dest_dir}/.vscode", dirs_exist_ok = True)

  # テンプレートのヘッダーファイルをコピーする
  if not os.path.isfile(f"{Path.cwd()}/templates/head_custom.html"):
    shutil.copyfile(f"{Path(__file__).parent}/templates/bootstrap.html", f"{dest_dir}/templates/head_custom.html")
...
def migrate(
  ...
  export_dir: str = typer.Option(..., "--export-dir", help="ファイルの保存先のディレクトリ"),
  date_from: str = typer.Option(
    datetime.strftime(datetime.today() - timedelta(days = 30), '%Y-%m-%d'), "--date-from", help="移行対象の開始日"
  ),
  date_until: str = typer.Option(
    datetime.strftime(datetime.today(), '%Y-%m-%d'), "--date-until", help="移行対象の終了日"
  ),
  ...
  migrate_repository.migrate(base_url, metadata_prefix, date_from, date_until, export_dir, metadata_only)
...
@app.command()
def validate(
  format: str = typer.Argument(..., help="メタデータのフォーマット")
):
  """
