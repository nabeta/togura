...
1. Toguraのzipファイルを以下のリンクからダウンロードし、適当なフォルダに展開します。ここでは`togura-main`フォルダに展開したものとします。  
    https://github.com/nabeta/togura/archive/refs/heads/main.zip
1. VSCodeを起動して、メニューから「ファイル」→「フォルダーを開く」を選び、展開したフォルダを選びます。フォルダの選択画面では、`togura-main`フォルダを選択（シングルクリック）した状態で「開く」ボタンを押してください。`togura-main`フォルダをダブルクリックしないでください。
