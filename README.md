...
- Toguraで扱うファイルは、一律で全体公開となります。ユーザ認証やアクセス元のIPアドレスによる限定公開機能はありません。
    - Webサーバの設定（`.htaccess`など）によってパスワード保護やIPアドレスによるアクセス制御をかけることは可能です。
...
1. VSCodeの画面左側のファイル一覧から`config.yml`を開き、以下の行を変更します。
    ...
    - `base_url: https://togura.example.ac.jp`と書かれている行のうち、`https://togura.example.ac.jp`という文字列を、公開先のWebサーバのトップページのURLに変更します。
        - Webサーバによる公開を行わず、自分のパソコンだけで動作を試す場合には、この項目は変更不要です。
