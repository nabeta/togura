...
1. `public`フォルダを開き、取り下げ対象のデータのフォルダを別のフォルダに移動、もしくは削除します。
1. 同様に`work`フォルダを開き、取り下げ対象のデータのフォルダを別のフォルダに移動、もしくは削除します。
1. 「リポジトリ用ファイルの出力」の手順に沿って、`togura.py generate`コマンドを実行します。
