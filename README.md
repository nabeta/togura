...
1. Windowsのエクスプローラーなどで、JPCOAR Schema Helperを保存したフォルダを開き、`work`フォルダの中に新しいフォルダを作成します。フォルダ名は資料名などわかりやすいものであれば、なんでもかまいません。ここでは`work`フォルダの中に`1001_my_article`フォルダを作ったこととして、以降そのフォルダを`work/1001_my_article`フォルダと記述します。
...
`work`フォルダをバックアップする必要があります。他のディスクなどにバックアップしておいてください。
