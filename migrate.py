...
def migrate(base_url, metadata_prefix, date_from, date_until, export_dir, metadata_only):
  ...
  for record in records:
    ...
    if record.xml.find(".//jpcoar:jpcoar/jpcoar:contributor", ns) is not None:
      contributors = []
      for contributor in record.xml.findall(".//jpcoar:jpcoar/jpcoar:contributor", ns):
        # 寄与者識別子
        ...
        if contributor.find("./jpcoar:contributorName", ns) is not None:
          for contributor_name in contributor.findall("./jpcoar:contributorName", ns):
    ...
    if record.xml.findall(".//jpcoar:jpcoar/jpcoar:rightsHolder", ns) is not None:
      rights_holders = []
      for rights_holder in record.xml.findall(".//jpcoar:jpcoar/jpcoar:rightsHolder", ns):
        # 権利者識別子
    ...
    if record.xml.find(".//jpcoar:jpcoar/jpcoar:publisher", ns) is not None:
      jpcoar_publishers = []
      for jpcoar_publisher in record.xml.findall(".//jpcoar:jpcoar/jpcoar:publisher", ns):
        # 出版者名
        # https://schema.irdb.nii.ac.jp/ja/schema/2.0/11-.1
        publisher_names = []
        for publisher_name in jpcoar_publisher.findall("./jpcoar:publisherName", ns):
          d = {"publisher_name": publisher_name.text}
          lang = publisher_name.get("{http://www.w3.org/XML/1998/namespace}lang")
          if lang is not None:
            d["lang"] = lang
        ...
        for publisher_description in jpcoar_publisher.findall("./jpcoar:publisherDescription", ns):
          d = {"publisher_description": publisher_description.text}
          lang = publisher_description.get("{http://www.w3.org/XML/1998/namespace}lang")
          if lang is not None:
            d["lang"] = lang
        ...
        publisher_locations = []
        for publisher_location in jpcoar_publisher.findall("./dcndl:location", ns):
          d = {"location": publisher_location.text}
          lang = publisher_location.get("{http://www.w3.org/XML/1998/namespace}lang")
          if lang is not None:
            d["lang"] = lang
        ...
        publisher_places = []
        for publisher_place in jpcoar_publisher.findall("./dcndl:publicationPlace", ns):
          d = {"publication_place": publisher_place.text}
          lang = publisher_place.get("{http://www.w3.org/XML/1998/namespace}lang")
          if lang is not None:
            d["lang"] = lang
    ...
    if record.xml.find(".//jpcoar:jpcoar/jpcoar:identifierRegistration", ns) is not None:
      registration = record.xml.find(".//jpcoar:jpcoar/jpcoar:identifierRegistration", ns)
      ...
      for relation in record.xml.findall(".//jpcoar:jpcoar/jpcoar:relation", ns):
        ...
        if relation.find("./jpcoar:relatedIdentifier", ns) is not None:
          ...
          if related_identifier.get("identifierType") is not None:
            d["related_identifier"]["identifier_type"] = related_identifier.get("identifierType")
    ...
    if record.xml.find(".//jpcoar:jpcoar/jpcoar:fundingReference", ns) is not None:
      funding_references = []
      for funding_reference in record.xml.findall(".//jpcoar:jpcoar/jpcoar:fundingReference", ns):
        d = {}
        # 助成機関識別子
        ...
        if funder_identifier:
          ...
          if funder_identifier.get("funderIdentifierTypeURI"):
            d["funder_identifier"]["funder_identifier_type_url"] = funder_identifier.get("funderIdentifierTypeURI")
        ...
        if award_number is not None:
          ...
          if award_number.get("awardURI"):
            d["award_number"]["award_number_url"] = award_number.get("awardURI")
          if award_number.get("awardNumberType"):
            d["award_number"]["award_number_type"] = award_number.get("awardNumberType")
    ...
    if record.xml.find(".//jpcoar:jpcoar/jpcoar:sourceIdentifier", ns) is not None:
      source_identifiers = []
      for source_identifier in record.xml.findall(".//jpcoar:jpcoar/jpcoar:sourceIdentifier", ns):
    ...
    if record.xml.find(".//jpcoar:jpcoar/dcndl:dissertationNumber", ns) is not None:
      dissertation_number = record.xml.find(".//jpcoar:jpcoar/dcndl:dissertationNumber", ns).text
    ...
    if record.xml.find(".//jpcoar:jpcoar/dcndl:degreeName", ns) is not None:
      degree_names = []
      for degree_name in record.xml.findall(".//jpcoar:jpcoar/dcndl:degreeName", ns):
    ...
    if record.xml.find(".//jpcoar:jpcoar/dcndl:dateGranted", ns) is not None:
      date_granted = record.xml.find(".//jpcoar:jpcoar/dcndl:dateGranted", ns).text
    ...
    if record.xml.find(".//jpcoar:jpcoar/jpcoar:conference", ns) is not None:
      conferences = []
      for conference in record.xml.findall(".//jpcoar:jpcoar/jpcoar:conference", ns):
        # 会議名
        # https://schema.irdb.nii.ac.jp/ja/schema/2.0/35-.1
        conference_names = []
        for conference_name in conference.findall("./jpcoar:conferenceName", ns):
          d = {"conference_name": conference_name.text}
          lang = conference_name.get("{http://www.w3.org/XML/1998/namespace}lang")
          if lang is not None:
            d["lang"] = lang
    ...
    if record.xml.find(".//jpcoar:jpcoar/dcndl:originalLanguage", ns) is not None:
      original_languages = []
      for original_language in record.xml.findall(".//jpcoar:jpcoar/dcndl:originalLanguage", ns):
    ...
    if record.xml.find(".//jpcoar:jpcoar/jpcoar:holdingAgent", ns) is not None:
      dataset_series = record.xml.find(".//jpcoar:jpcoar/jpcoar:datasetSeries", ns).text
